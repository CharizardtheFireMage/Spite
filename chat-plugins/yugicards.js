...
exports.commands = {
	...
	duelsearch: function (target, room, user) {
		...
		const generalMenu = "<center>" +
			'<button name="send" value="/duelsearch letter" style=\"background-color:aliceblue;height:30px\">Alphabetical</button>&nbsp;&nbsp;' + // alphabetical
			'<button name="send" value="/duelsearch category" style=\"background-color:aliceblue;height:30px\">Categories</button>&nbsp;&nbsp;' + // category
			'</center><br />';
		if (!target) {
		...
		case 'letter':
			...
			const letterMenu = '<center>' + letters.map(l => {
				return '<button name="send" value="/duelsearch letter, ' + l + '" ' + (letter === l ? "style=\"background-color:lightblue;height:30px;width:35px\"" : "style=\"background-color:aliceblue;height:30px;width:35px\"") + ">" + l.toUpperCase() + "</button>";
			}).join("&nbsp;") + "</center><br />";
			...
			dualcardDisplay = Object.keys(letterMons[letter]).sort().map(m => {
				...
				return '<button name="send" value="/duelsearch duelcard, ' + dualcard.title + '" style="border-radius: 12px; box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.2) inset;" class="card-button"><img src="' + dualcard.card + '" width="100" title="' + dualcard.name + '"></button>';
			}).join("&nbsp;");
			// send the popup
		...
		case 'category':
			...
			for (let c in categories) {
				categoryMenu += '<b>' + c + ' -</b> ' + categories[c].map(k => {
					...
					return '<button name="send" value="/duelsearch category, ' + newParams.join(", ") + '" ' + style + '>' + k + '</button>';
				}).join("&nbsp;") + "<br />";
			}
			if (!parts.length) {
