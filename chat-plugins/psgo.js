...
exports.commands = {
	...
	buypack: function (target, room, user) {
		...
		if (packId !== 'xybase' && packId !== 'xyfuriousfists' && packId !== 'xyflashfire' && packId !== 'xyphantomforces' && packId !== 'xyroaringskies' && packId !== 'xyprimalclash' && packId !== 'xyancientorigins' && packId !== 'xygenerations' && packId !== 'xyfatescollide' && packId !== 'xybreakpoint' && packId !== 'xypromo' && packId !== 'bwplasmablast') return self.sendReply("This pack is not currently in circulation.  Please use /packshop to see the current packs.");
	...
	searchcard: function (target, room, user) {
		...
		const categories = {
			...
			Packs: ['XY-Promo', 'XY-Base', 'XY-Flashfire', 'XY-Furious Fists', 'XY-Phantom Forces', 'XY-Primal Clash', 'XY-Roaring Skies', 'XY-Ancient Origins', 'Double Crisis', 'XY-Generations', 'XY-Fates Collide', 'XY-BREAKpoint', 'BW-Plasma Blast'],
